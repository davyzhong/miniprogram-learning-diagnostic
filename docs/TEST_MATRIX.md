...
> 当前单元自动化基线：`npm test` / `npm run test:unit` 共 638 个用例通过
...
| 上传照片、报告创建、分析任务启动 | `cloud-functions.test.js`、`analyze-photos-pipeline.test.js`、`poller.test.js` | 真实图片需 `test:e2e:real-image` | 本地链路已覆盖，真机拍照需人工验收 |
...
| 诊断报告展示和报告质量 | `report-presenter.test.js`、`report-quality.test.js`、`report-page-flows.test.js` | `test:e2e:core`、`test:e2e:math` | 已覆盖 |
