...
> **⚠ 已过时（2026-07-10）**：本文档中的 P0-1（照片分析 batch size=1）和 P0-2（验证卷生成 batch size=1）已在后续版本中修复（batch size 改为 5）。文档中的统计数字（13 云函数、19 页面、10 集合）也已过时。当前数据见 docs/ARCHITECTURE.md。
...
```
N 张照片 = N 次串行 `analyzeBatch` AI 调用 + N 次 DB 进度写入。S1的 160 张试卷照片意味着 **160 次串行 AI 调用**，每次 ≤60s 超时。
