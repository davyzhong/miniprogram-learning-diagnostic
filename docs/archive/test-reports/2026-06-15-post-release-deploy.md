...
本机未配置 `REAL_DATA_STUDENT_ID`，因此本轮没有执行真实学生 ID 定位的 `npm run test:real-data-smoke`。拿到S1真实学生 `_id` 后可继续执行：

```bash
REAL_DATA_STUDENT_ID=<真实学生ID> REAL_DATA_STUDENT_NAME=S1 npm run test:real-data-smoke
```
