...
**内置个人词库**：`data/english/case-s1-pep-vocabulary.seed.json` 是S1当前英语学习使用的 PEP 个人词库归档，覆盖 3上、3下、4上、4下、5上、5下、6上，共 505 条单词/教材词组。云函数部署包内的副本位于 `cloudfunctions/englishVocabulary/case-s1-pep-vocabulary.json`。
...
| `generationProgress` | Object | 否 | `null` | 分批生成进度（v2）：`{ completedBatches, totalBatches, succeededBatches, failedBatches?, failedBatchIndexes? }`，供前端轮询展示 | `{"completedBatches":3,"totalBatches":5,"succeededBatches":3,"failedBatches":0}` |
...
| targets 格式 | generatePaper | 验证卷兼容 `LP-*` 粗卡点、`BN-*` 细卡点、`CHI-*` 语文错项目标，任务包模式最多 80 个；默认诊断卷不使用 targets |
...
| 内置词库导入 | englishVocabulary.seedPersonalVocabulary | 仅接收 `studentId`；固定使用项目内置S1 PEP 个人词库 |
