...
| 命令 | 用途 | 备注 |
|------|------|------|
| `npm test` | 运行常规自动化测试 | 显式运行 15 个常规测试文件，不包含真实图片 E2E |
...
| `contracts.test.js` | 跨模块契约、命名一致性、已修复缺陷回归保护 | 23 |
...
| `page-flows.test.js` | 10 个页面的主流程、错误恢复、导航跳转 | 27 |
