...
3. 截图匿名姓名白名单固定为 `学生示例`、`孩子A`、`家长A`；已知真实姓名拒绝清单为 `S1`、`S2`，并允许通过 `README_SCREENSHOT_FORBIDDEN_NAMES` 追加。截图生成器对 URL 解码后的路由和渲染文本使用同一组规则，拒绝 `cloud://`、`wxfile://`、`LP-`、`BN-`、openid、手机号、学校/班级/账号字段、24 位数据库 ID 和 UUID；允许家长可读的“学科-YYYYMMDD-序号”验证卷编号。另对 14 张输出逐张目视复核。
