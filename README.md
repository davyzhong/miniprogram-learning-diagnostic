...
多个孩子时，首页会显示家庭学习工作台；只有一个孩子时，会直接进入该孩子的学习档案。卡片里的数字、状态、学科和最近试卷都可以点击，直接进入对应页面。
...
学习档案是单个孩子的总览页，最重要的是看“最新诊断报告”和“当前学习卡点”。这里会显示报告生成时间、证据时间、相关错题数和下一步建议。
...
| 区域 | 用途 |
|------|------|
| 当前综合摘要 | 快速判断这个孩子目前有没有形成学习线索 |
| 最新数学诊断报告 | 查看最近一次 AI 诊断结论、照片数和相关错题数 |
| 当前学习卡点 | 看哪些卡点待跟进、持续出现或已经改善 |
| 学科入口 | 进入数学、语文、英语的具体工作台 |
...
验证试卷用于确认学习卡点是否真实存在、是否已经改善。当前规则是每个学习卡点 5 道题：3 道核心验证题 + 2 道迁移延展题。
