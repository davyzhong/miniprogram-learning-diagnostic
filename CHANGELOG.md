...
- 家庭与个人学习工作台重构：家庭页增加图示总览入口，个人页增加个人插图、今日行动、最新诊断报告、行动队列和三科学科入口，所有卡片均可点击进入真实页面
- 页面主题插图体系：新增数学、语文、英语、诊断报告、验证报告、验证卷、知识地图、学习资源、上传、学习记录和英语练习等 13 张轻量插图，并接入主要学习与管理页面
...
- `npm test` 当前运行 636 个常规自动化用例；真实图片和 DevTools E2E 脚本按需单独运行
...
- 页面主题插图显示异常：主题插图统一改为完整展示，并调整横版插图容器比例，避免只显示局部或缩得过小。
- 页面主题插图比例统一：将主要学习/管理页面的装饰场景图统一为 `1280×720` 素材与 16:9 全宽容器，避免 `aspectFit` 留白导致画面不贴合。
