...
**创建日期**: 2026-06-09
**最后更新**: 2026-09-12
**项目状态**: 私有内测持续迭代；数学、语文具体错项复测和英语词汇双维闭环已落地。默认离线测试 1110/1110、JS 检查 351 个文件、ESLint 0 error、主包 810 KB/1200 KB（预算 2026-07-18 由 800 上调）；Android/iOS emoji 目标设备兼容基线已固化。**质量基线数字的唯一来源是 README「当前质量基线」表，本文件不再复制具体数字**
**负责人**: qiming
...
本项目是学习诊断 MVP 的微信小程序产品。此前已通过纯 AI 流程完成了S1的学习卡点诊断（第一版 + 第二版，共分析 132 张试卷图片、80+ 道错题、10 大类卡点），并生成了诊断报告、验证卷等纸质材料。
...
│   ├── app.json                     # 全局配置（27 个页面路由：主包 8 + 分包 19）
│   ├── app.wxss                     # 全局样式
│   ├── sitemap.json                 # 站点地图配置
│   │
│   ├── utils/
...
**工程基线**: 347 个 JavaScript 文件（`npm run check` 校验）；仓库有 99 个 `.test.js` 文件，默认离线集显式执行其中 94 个并通过 1110 个用例。其余为真实云/真实图片 E2E 和专项数学管线测试，按需单独运行。

### 2.2 相关文件索引

| 文件 | 用途 |
|------|------|
| `学习卡点诊断报告_第二版.md` | S1完整诊断报告（132 张图 / 80+ 错题 / 10 大类） |
...
| `englishVocabulary` | 管理S1个人英语词库，生成 20 词听写队列，接收语音识别结果并更新掌握度和复测节奏 |
