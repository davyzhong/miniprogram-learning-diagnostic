...
test('C01 is the frozen first verified snapshot without approving later candidates', () => {
  ...
  assert.deepEqual(Object.values(UI_SYMBOLS), c01.items.map(item => item.glyph))
  EMOJI_CATEGORIES.slice(1).flatMap(category => category.items).forEach(item => {
