...
const seed = require('../data/english/case-s1-pep-vocabulary.seed.json')
const cloudSeed = require('../cloudfunctions/englishVocabulary/case-s1-pep-vocabulary.json')
...
test('Zhong Qingyu PEP English vocabulary seed is complete enough for dictation', () => {
  assert.equal(seed.studentName, 'S1')
...
test('submitRecognitionAttempt rejects word that does not belong to the student', async () => {
  const db = createDatabase({
    students: [
      { _id: 'student-1', _openid: 'owner-1', name: 'S1', grade: 6 },
