...
test('add student trims input and creates all subject profiles', async () => {
  ...
  page.onNameInput({ detail: { value: '  S1  ' } })
...
test('single-child index opens directly as that child learning profile', async () => {
  const cloud = {
    getAccessibleStudents: async () => [
      { _id: 'student-1', name: 'S1', grade: 5, createdAt: '2026-06-01' }
    ],
    getStudentDashboard: async studentId => ({
      student: { _id: studentId, name: 'S1', grade: 5 },
  ...
  assert.equal(page.data.home.studentName, 'S1')
  assert.equal(page.data.childCards.length, 0)
  page.onStudentTap({ currentTarget: { dataset: { id: 'student-1', name: 'S1', grade: 5 } } })
...
test('multi-child index shows only the family workbench and routes child cards to profile pages', async () => {
  const cloud = {
    getAccessibleStudents: async () => [
      { _id: 'student-1', name: 'S1', grade: 6, createdAt: '2026-06-01' },
      { _id: 'student-2', name: '弟弟', grade: 3, createdAt: '2026-06-02' }
    ],
    getStudentDashboard: async studentId => {
      const name = studentId === 'student-1' ? 'S1' : '弟弟'
  ...
  page.onStudentTap({ currentTarget: { dataset: { id: 'student-1', name: 'S1', grade: 6 } } })
  assert.match(wx.calls.find(call => call.name === 'navigateTo').payload.url, /pages\/student-profile\/student-profile/)
})
...
test('family page renders all three subjects and four quick actions exactly once', () => {
  const [card] = buildChildWorkbenchCards({
    students: [{ _id: 'student-cardinality', name: 'S1', grade: 6 }]
  })
  const wxml = fs.readFileSync(path.join(ROOT, 'miniprogram/pages/index/index.wxml'), 'utf8')
...
test('learning profile home loads the active student summary', async () => {
  const cloud = {
    ...
    getStudentDashboard: async studentId => {
      ...
      return {
        student: { _id: studentId, name: 'S1', grade: 6 },
        permissions: { canView: true, canUpload: true, canGeneratePaper: true, canRetryAnalysis: true, canManageParents: true },
        subjectProfiles: [{
          subject: 'math',
          subjectName: '数学',
...
test('learning profile home opens parent management for the active student', async () => {
  ...
  page.setData({
    ...
    activeStudent: { _id: 'student-1', name: 'S1' },
  ...
  const cloud = {
    getHomeDashboard: async () => {
      ...
      return {
        children: [
          {
            student: { _id: 'student-1', name: 'S1', grade: 6, permissions: { canView: true } },
            role: 'owner',
            permissions: { canView: true, canManageParents: true, canUpload: true, canGeneratePaper: true },
          ...
          {
            student: { _id: 'student-2', name: 'S2', grade: 3, permissions: { canView: true } },
            role: 'viewer',
            permissions: { canView: true, canManageParents: false, canUpload: true, canGeneratePaper: true },
            subjectProfiles: [],
            recentReports: [],
...
test('index onShow reuses a fresh dashboard snapshot instead of refetching immediately', async () => {
  ...
  const cloud = {
    getAccessibleStudents: async () => {
      accessibleCalls += 1
      return [{ _id: 'student-1', name: 'S1', grade: 6 }]
    },
    getStudentDashboard: async studentId => {
      dashboardCalls += 1
      return {
...
test('index uses getHomeDashboard as a single aggregated call when available', async () => {
  ...
  const cloud = {
    getHomeDashboard: async () => {
      ...
      return {
        students: [
          { _id: 'student-1', name: 'S1', grade: 5, createdAt: '2026-06-01', role: 'owner', permissions: { canView: true, canUpload: true, canGeneratePaper: true, canRetryAnalysis: true, canManageParents: true } },
          { _id: 'student-2', name: 'S2', grade: 3, createdAt: '2026-06-02', role: 'owner', permissions: { canView: true, canUpload: true, canGeneratePaper: true, canRetryAnalysis: true, canManageParents: true } }
        ],
        perStudent: {
          'student-1': {
...
test('index falls back to 1+N path when getHomeDashboard throws', async () => {
  ...
  const cloud = {
    ...
    getAccessibleStudents: async () => { accessibleCalls++; return [{ _id: 'student-1', name: 'S1', grade: 5, createdAt: '2026-06-01' }] },
    getStudentDashboard: async () => ({
      subjectProfiles: [],
      recentReports: [],
      recentPapers: []
...
test('child workbench cards expose status segments and a learning-progress entry for improved', () => {
  const [card] = buildChildWorkbenchCards({
    students: [{ _id: 'student-segments', name: 'S1', grade: 6 }],
    profilesByStudentId: {
      'student-segments': [{
        subject: 'math',
        currentBottlenecks: [
...
test('child card improved metric falls back to bottleneck center without improvements', () => {
  const [card] = buildChildWorkbenchCards({
    students: [{ _id: 'student-no-improved', name: 'S1', grade: 6 }],
    profilesByStudentId: {
      'student-no-improved': [{
        subject: 'math',
...
test('family hero exposes a kicker symbol and an aggregated status bar', () => {
  const cards = buildChildWorkbenchCards({
    students: [
      { _id: 'student-a', name: 'S1', grade: 6 },
      { _id: 'student-b', name: 'S2', grade: 3 }
    ],
    profilesByStudentId: {
