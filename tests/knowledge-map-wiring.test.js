...
//   - student-profile.wxml 里新增的 onKnowledgeMapTap 在 .js 里有定义
...
// ============================================================
// 2. student-profile：新增的 onKnowledgeMapTap 必须在 .js 里有定义
// ============================================================

test('student-profile.wxml 用到的 onKnowledgeMapTap 在 .js 中已定义', () => {
  const wxml = read('miniprogram/pages/student-profile/student-profile.wxml')
  const js = read('miniprogram/pages/student-profile/student-profile.js')

  assert.match(wxml, /onKnowledgeMapTap/, 'wxml 必须绑定 onKnowledgeMapTap')
  assert.match(js, /onKnowledgeMapTap\s*\(/, 'js 必须定义 onKnowledgeMapTap 方法')
})

test('student-profile.wxml 渲染 knowledgeMapCard 的紧凑条字段', () => {
  const wxml = read('miniprogram/pages/student-profile/student-profile.wxml')
  // 紧凑条方案：不再用 domains，改用 summary 单行展示
  const requiredBindings = [
    'home.knowledgeMapCard.visible',
    'home.knowledgeMapCard.summary',
  ]
  const missing = requiredBindings.filter(b => !wxml.includes(b))
  assert.deepEqual(missing, [], `student-profile.wxml 缺少这些绑定: ${missing.join(', ')}`)
  // 紧凑条应使用 map-compact-bar class（不再是 section-block + map-card）
  assert.match(wxml, /map-compact-bar/, '应使用紧凑条 class')
})
