...
async function installCloudMocks(miniProgram) {
  await miniProgram.evaluate(() => {
    ...
    const student = { _id: 'student-e2e', name: 'S1', grade: 6 }
...
async function main() {
  ...
  await runCase('家庭工作台真实加载：多个孩子只显示工作台', async () => {
    ...
    requireText(text, ['家庭学习工作台', '点击数字、状态、学科和编号可直接追溯', 'S1', '钟小羽', '待验证', '待上传'])
  ...
  await runCase('点击孩子卡片主体：进入该孩子学习档案', async () => {
    ...
    await tapByText(page, '.child-card-top', 'S1')
  ...
  await runCase('点击家长管理：进入家庭成员页面', async () => {
    ...
    const text = await pageText(page)
    requireText(text, ['家庭成员', 'S1', '已加入家长', '档案创建者', '共同查看家长', '邀请家长', '生成邀请'])
  ...
  await runCase('学科选择页：三科入口可见并可进入数学工作台', async () => {
    let page = await relaunch(miniProgram, '/pages/subject-select/subject-select?studentId=student-e2e&name=S1&grade=6', 1600)
    let text = await pageText(page)
    requireText(text, ['S1 的学科入口', '数学', '语文', '英语'])
    await tapByText(page, '.subject-card', '数学')
    await page.waitFor(1600)
    page = await miniProgram.currentPage()
    assert.equal(page.path, 'pages/subject-home/subject-home')
    text = await pageText(page)
    requireText(text, ['下一步建议', '生成验证试卷', '待处理队列', '工具', '拍照诊断', '默认试卷', '学习记录', '完整报告'])
    await miniProgram.screenshot({ path: screenshots.subject })
  })

  await runCase('学科主页工具：拍照上传、默认试卷、学习记录入口可用', async () => {
    let page = await relaunch(miniProgram, '/pages/subject-home/subject-home?studentId=student-e2e&subject=math&subjectName=%E6%95%B0%E5%AD%A6&studentName=S1&grade=6', 1600)
    ...
    page = await relaunch(miniProgram, '/pages/subject-home/subject-home?studentId=student-e2e&subject=math&subjectName=%E6%95%B0%E5%AD%A6&studentName=S1&grade=6', 1600)
  ...
  await runCase('生成验证试卷页：卡点文字、配置和按钮可见', async () => {
    const page = await relaunch(miniProgram, '/pages/generate-verification/generate-verification?studentId=student-e2e&subject=math&subjectName=%E6%95%B0%E5%AD%A6&studentName=S1', 1800)
    const text = await pageText(page)
  ...
  await runCase('卡点中心筛选与卡点详情：文字摘要、证据链和验证入口可见', async () => {
    let page = await relaunch(miniProgram, '/pages/bottleneck-center/bottleneck-center?studentId=student-e2e&studentName=S1', 1600)
  ...
  await runCase('时间线点击验证反馈：报告详情显示关联试卷编号', async () => {
    let page = await relaunch(miniProgram, '/pages/upload-history/upload-history?studentId=student-e2e&studentName=S1', 1600)
  ...
  await runCase('时间线点击照片：触发照片预览入口不报错', async () => {
    ...
    const page = await relaunch(miniProgram, '/pages/upload-history/upload-history?studentId=student-e2e&studentName=S1', 1600)
