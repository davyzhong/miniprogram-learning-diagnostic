...
> 版本：v2.5 | 日期：2026-06-12 | 状态：MVP 编码完成，常规自动化测试 136/136 通过，JS 语法检查 52 文件通过，待真机验收
...
| 指标 | 学习观察数、有效报告数、最近更新或已改善数 |
| 学习观察 | 按学科汇总当前待验证或已有改善的观察，不展示裸 LP 编号 |
| 学习记录 | 最近上传照片、诊断报告、验证试卷和验证反馈，支持查看全部 |
| 下一步建议 | 根据是否有待验证观察，推荐生成验证试卷或上传新试卷 |
| 学科入口 | 数学/语文/英语三科入口，进入单学科详情 |

**交互**：点击学习观察或学科入口 → 进入 Page 3；点击学习记录 → 进入报告或试卷预览；点击「查看全部」→ 进入 Page 4A；点击「管理孩子」→ 跳转 `add-student` 页。
...
| 功能 | 页面 | 说明 |
|------|------|------|
| 学习档案首页 | Page 1 | 综合摘要、样本覆盖、学习观察、学习记录、下一步建议 |
...
| 学习档案首页 | ✅ | `index-presenter.js` 聚合综合摘要、样本覆盖、学习观察、学习记录和下一步建议 |
...
| 自动化测试覆盖（136 常规用例全绿） | ✅ | `npm test`；真实图片 E2E 脚本需通过 `npm run test:e2e-real-image` 单独运行 |
